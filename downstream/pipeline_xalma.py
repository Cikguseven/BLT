...
def build_fewshot_prompt(
    ...
    if n_shots > 0 and tgt_lang in _SHOT_TGT:
        # Instruction appears ONCE on the first shot only
        first_s = _SHOT_EN[0]
        ...
        # Remaining shots — bare Src/Tgt pairs, no repeated header
        for s, t in zip(_SHOT_EN[1:n_shots], _SHOT_TGT[tgt_lang][1:n_shots]):
            parts.append(
                f"{src_name}: {s}\n"
                f"{tgt_name}: {t}"
            )
        # Final test block — no repeated header
        parts.append(
            f"{src_name}: {test_src}\n"
            f"{tgt_name}:"
        )
    else:
        # Zero-shot fallback — instruction only here
        parts.append(
            f"Translate this from {src_name} to {tgt_name}:\n"
            f"{src_name}: {test_src}\n"
            f"{tgt_name}:"
...
def clean_generation_text(text) -> str:
    # Flatten any list/nested-list the BLT decoder may return
...
# All English language names used as stop labels during extraction.
...
def extract_translation(
    ...
    # Normalise input — BLT decoder can return list[str] or list[list[str]]
    ...
    if not lines:
        ...
        out = lines[0]
        # Final safety net — ensure out is always a str before regex
...
def run_inference_hf(
    ...
    return_confidence: bool = True,          # ← new
) -> tuple[list[str], dict | None]:          # ← now returns (outputs, confidence)
    outputs = []
    truncation_count = 0
    input_device = _get_model_input_device(model)

    all_probs_corpus:  list[float] = []
    all_logits_corpus: list[float] = []
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        gen_output = model.generate(
            **enc,
            ...
            output_scores=return_confidence,              # ← new
            return_dict_in_generate=return_confidence,    # ← new
        )

        if return_confidence:
            gen_ids = gen_output.sequences           # (B, input_len + gen_len)
            scores  = gen_output.scores              # tuple[(B, V)] × gen_len
        else:
            gen_ids = gen_output
        ...
        for j in range(gen_ids.size(0)):
            ...
            if return_confidence:
                ...
                for t, step_scores in enumerate(scores):
                    tok_id = gen_ids[j, input_len + t].item()
                    if tok_id == hf_tok.pad_token_id:
                        continue
                    logit = step_scores[j, tok_id].item()
                    prob  = F.softmax(step_scores[j], dim=-1)[tok_id].item()
                    all_probs_corpus.append(prob)
                    all_logits_corpus.append(logit)
                    if tok_id == eos_id:
                        break
    ...
    confidence = None
    if return_confidence and all_probs_corpus:
        confidence = {
            "mean_prob"  : round(sum(all_probs_corpus)  / len(all_probs_corpus),  6),
            "max_prob"   : round(max(all_probs_corpus),                            6),
            "mean_logit" : round(sum(all_logits_corpus) / len(all_logits_corpus),  4),
            "max_logit"  : round(max(all_logits_corpus),                           4),
        }
...
def run_inference(
    ...
    if "blt" in tokenizer_name.lower():
        # BLT does not expose per-step scores; confidence is unavailable
        raw = run_inference_blt(
            model=model,
            blt_tok=tokenizer,
            patcher=patcher,
...
def evaluate_direction(
    ...
    # Build bare X-ALMA prompts, then wrap in chat template
    ...
    raw_out, confidence = run_inference(     # ← unpack tuple
    ...
    tgt_label = LANG_EN_NAMES[tgt_lang]  # e.g. "Indonesian", "Thai", …
    ...
    return {
        ...
        "bleu": round(bleu, 4),
        "chrf": round(chrf, 4),
        "mean_prob"  : confidence["mean_prob"]   if confidence else None,   # ← new
        "max_prob"   : confidence["max_prob"]    if confidence else None,   # ← new
        "mean_logit" : confidence["mean_logit"]  if confidence else None,   # ← new
        "max_logit"  : confidence["max_logit"]   if confidence else None,   # ← new
...
def run_full_evaluation(
    ...
    fieldnames = [
        ...
        "bleu", "chrf",
        "mean_prob", "max_prob", "mean_logit", "max_logit",   # ← new
    ]
    ...
    print(f"  {'Direction':<12} {'BLEU':>8} {'chrF++':>8} {'MeanProb':>10} {'MaxProb':>9} {'MeanLogit':>10} {'MaxLogit':>9} {'Empty':>7}")
    print("  " + "-" * 78)
    for r in results:
        mp  = f"{r['mean_prob']:.4f}"  if r['mean_prob']  is not None else "  N/A  "
        xp  = f"{r['max_prob']:.4f}"   if r['max_prob']   is not None else "  N/A  "
        ml  = f"{r['mean_logit']:.4f}" if r['mean_logit'] is not None else "  N/A  "
        xl  = f"{r['max_logit']:.4f}"  if r['max_logit']  is not None else "  N/A  "
        print(f"  {r['direction']:<12} {r['bleu']:>8.4f} {r['chrf']:>8.4f} {mp:>10} {xp:>9} {ml:>10} {xl:>9} {r['num_empty_hypotheses']:>7}")
...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="/localhome/kieron/fyp/mt_results")
    ...
    parser.add_argument("--en_xx_only", action="store_true",
